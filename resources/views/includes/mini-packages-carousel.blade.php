<div id="carousel-mini-courses" class="owl-carousel owl-theme clearfix">
    @foreach ($packages['data'] as $package)
        <div class="item mt-3 mb-4 w-100 popup-trigger" data-popover-id="mini-course-popover-{{ $package['id'] }}">
            <div class="w-100 course-popover-trigger">
               <a href="{{ url('packages/' . ($package['slug'] ?? $package['id'])) }}" class="text-dark text-decoration-none w-100">
                ...
                <div id="mini-course-popover-{{ $package['id'] }}"
                     class="popup w-100">
    ...
    @if(count($packages['data']) >= 11)
        <div class="item mx-2 mt-3 mb-4 popup-trigger" data-popover-id="mini-course-popover-1000">
            <div class="course-popover-trigger">
                <a href="{{ url('packages?package_type=mini')}}" class="text-dark text-decoration-none">
                    <div class="card shadow">
                        <div class="card-body d-flex flex-column">
...
@push('script')
    <script>
        $(document).ready(function () {
            $('#carousel-mini-courses').owlCarousel({
                margin: 20,
                loop: false,
                nav: {{ $nav ?? 'true' }},
                dots: {{ $nav ?? 'false' }},
