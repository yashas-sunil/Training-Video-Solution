                                ...
                                <div class="course_content">
                                    <div class="content_header">
                                        ...
                                        <div class="content_list mt-2">
                                            ...
                                            $valid = date('Y-m-d h:i:s', strtotime($studyMaterials['created_at'] . "+$freemiumDaysMax days"));
...
$duration =(@$hrs[0]*@$studyMaterials['package']['duration'])*3600;
$diffinsec=(@$duration- @$watchedduration)/3600;
$total_duration=(@$hrs[0]*@$studyMaterials['package']['duration']);
...
$totaldurationinsec=$time_seconds*((float) $studyMaterials['package']['duration']);
...
$remainingDurationInHrs=$hours1.':'.$minutes1.':'.$seconds1;
                                            ...
                                            Valid till: {{date('d F Y', strtotime($studyMaterials['created_at'] . "+$freemiumDaysMax days"))}} </h6>
                                            <span></span>
                                                                            ...
                                                                            <div class="inner_module">
                                                                                ...
                                                                                <ul class="module_diff">
                                                                                    @foreach($module['videos'] as $moduleVideo)
                                                                                        <?php
                                                                                            $non_allowed_class = "";
                                                                                            $disp_videoId = $moduleVideo['id'];
                                                                                            if(!empty($allowed_videos) && !in_array($moduleVideo['id'],$allowed_videos)){
                                                                                                $non_allowed_class = 'video-expired';
                                                                                                $disp_videoId = '';
                                                                                            }
                                                                                        ?>
                                                                                        ...
                                                                                        <li id="last-video-played-{{$moduleVideo['id']}}" class="last-video-played lecture_list @if($moduleVideo['id'] == $currentVideoId) continue @else play_next @endif">
                                                                                            ...
                                                                                            <a class="text-info video-link {{ $non_allowed_class }}" href="#" data-video-id="{{ $disp_videoId }}" data-video-title="{{ $moduleVideo['title'] }}">
                                                                                                ...
                                                                                                @if(count($moduleVideo['video_histories'])>0)
                                                                                                    @if($moduleVideo['video_histories'][0]['position'] != 0)
                                                                                                        <div id="progress-{{ $moduleVideo['id'] }}" class="lecture_progress progress-bar d-none" style=" position: absolute;
                                                                                                            width: {{ number_format($moduleVideo['video_histories'][0]['position'] *100 / $moduleVideo['video_histories'][0]['total_duration']) }}%;
                                                                                                            left: 0;
                                                                                                            right: 0;
                                                                                                            height: 100%;
                                                                                                            background: #58CEA4;
                                                                                                ...
                                                                                                <div class="lecture_head">
                                                                                                    @if(empty($non_allowed_class))
                                                                                                    <span style="font-size:12px;display:flex;line-height:15px;"><p class="last_played" style="margin-right:10px;">Now Playing</p>Free Trial</span>
                                                                                                    @else
                                                                                                    ...
                                                                                                    <ul class="module_views">
                                                                                                        ...
                                                                                                        <li>
                                                                                                            <h6 id="percentage-{{$moduleVideo['id']}}">
                                                                                                                @if(count($moduleVideo['video_histories'])>0)
                                                                                                                    @if($moduleVideo['video_histories'][0]['position'] != 0)
                                                                                                                        {{ number_format($moduleVideo['video_histories'][0]['position'] *100 / $moduleVideo['video_histories'][0]['total_duration']) }}% Completed
                                                                                                                    @else
                                                                                                                        0% Completed
                                                                                                                    @endif
                                                                                                                @else
        ...
        $(function() {
            ...
            function getPlayer(videoID) {
                $.ajax({
                    ...
                    $.ajax({
                        url: '{{ url('freemium-video-histories') }}',
                        data: {
                            package_id: '{{ $package['id'] }}',
                            video_id: videoID,
                            freemium_package_id: {{ $freemiumPackage }}
                        },
                        async: false
                    }).done(function(response) {
                        var positionValue;
                    ...
                    $.ajax({
                        url: '{{ url('freemium-remaining-duration') }}',
                        data: {
                            package_id: '{{ $package['id'] }}',
                            freemium_package_id: '{{ $freemiumPackage }}'
                        },
                    ...
                    @endif
                        ...
                        document.addEventListener("keydown", function (event) {
                            // Check if the space bar key is pressed (key code 32)
                            switch (event.keyCode) {
                                case 32: // Space bar
                                    // Prevent the default behavior of the space bar (scrolling the page)
...
// trim version
            ...
            $('.video-link').click(function() {
                if($(this).hasClass('video-expired')){
                    if(videoType === 's3'){
                        var player = videojs('video-player-js');
                        player.pause();
                    }else{
...
//                var $floater = $('#video_floater');
                if(videoType != undefined && videoType == "s3"){
